 ...
 */
function calculateEntropyScore(secret: string): number {
 ...
 */
export function validateJwtSecret(secret: string | undefined): ValidationResult {
  const errors: string[] = [];
  ...
  if (WEAK_SECRETS.has(secret.toLowerCase())) {
    errors.push(
      `JWT_SECRET is a known weak/default value ("${secret.slice(0, 10)}..."). ` +
  ...
  if (errors.length === 0 && entropyScore < 30) {
    warnings.push(
      'JWT_SECRET entropy is acceptable but could be improved. ' +
...
export function validateEnv(options: EnvValidationOptions = {}): void {
  ...
  if (skipValidation) {
    console.warn(
      `[${serviceName}] WARNING: Environment validation is disabled. ` +
